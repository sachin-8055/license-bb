...
function aesGenerateKeys() {
  try {
    const newAesKey = crypto.randomBytes(defaultKeyLength).toString("base64");
    return newAesKey || "";
  } catch (error) {
...
function aesEncrypt(secretKey = "", plainText = "") {
  try {
    ...
    const cipher = crypto.createCipheriv(algorithum, aesKey, null);
...
function aesDecrypt(secretKey = "", encryptedText = "") {
  try {
    ...
    let aesKey = Buffer.from(secretKey, "base64");
    const decipher = crypto.createDecipheriv(algorithum, aesKey, null);
...
var logging = (org_Id = "", reason = "", result = "") => __async(void 0, null, function* () {
  if (fs2) {
    if (fs2.existsSync(`${baseFolderPath}/${logFile}`)) {
      try {
        const existingData = fs2.readFileSync(`${baseFolderPath}/${logFile}`, "utf8");
        const newData = `${(/* @__PURE__ */ new Date()).toISOString()} > ${org_Id}: ${reason}: ${result}`;
        const updatedData = `${existingData.trim()}
${newData}`;
        fs2.writeFileSync(`${baseFolderPath}/${logFile}`, updatedData);
      } catch (err) {
...
var getTrace = (org_Id = "") => __async(void 0, null, function* () {
  if (fs2) {
    if (fs2.existsSync(`${baseFolderPath}/${org_Id}/${infoTracerFile}`)) {
      let traceFileData = fs2.readFileSync(`${baseFolderPath}/${org_Id}/${infoTracerFile}`, "utf8");
      if (traceFileData) {
...
var updateTrace = (org_Id = "", JsonData) => __async(void 0, null, function* () {
  if (fs2) {
    ...
    if (oldTrace && oldTrace !== null && JsonData) {
      ...
      fs2.writeFileSync(`${baseFolderPath}/${org_Id}/${infoTracerFile}`, JSON.stringify(newTraceData, null, 2));
    } else if (!oldTrace && JsonData) {
      fs2.writeFileSync(`${baseFolderPath}/${org_Id}/${infoTracerFile}`, JSON.stringify(JsonData, null, 2));
    }
  }
});
...
var getDeviceDetails = () => __async(void 0, null, function* () {
  ...
  if (fs2.existsSync(filePath)) {
    let fileData = fs2.readFileSync(`${baseFolderPath}/${deviceFile}`, "utf8");
    const parseData = fileData ? JSON.parse(fileData) : null;
    if (parseData) {
      return parseData || null;
    }
  }
  const platform = (process == null ? void 0 : process.platform) || os.platform();
  const _host = ((_a = process == null ? void 0 : process.env) == null ? void 0 : _a.HOSTNAME) || os.hostname();
  ...
  if (fs2.existsSync("/proc/1/cgroup")) {
    fs2.readFile("/proc/1/cgroup", "utf8", (err, data) => {
      if (err) {
  ...
  try {
    fs2.writeFileSync(`${baseFolderPath}/${deviceFile}`, JSON.stringify(_deviceDetails, null, 2));
  } catch (error) {
...
var _License = class {
    ...
    return __async(this, null, function* () {
      ...
      try {
        ...
        if (!preChecks.isPublicFile || !preChecks.isPrivateFile) {
          ...
          fs2.writeFileSync(`${baseFolderPath}/${org_Id}/temp_${publicFile}`, keyGen.publicKey);
          fs2.writeFileSync(`${baseFolderPath}/${org_Id}/temp_${privateFile}`, keyGen.privateKey);
          isExchangeNow = true;
        }
        if (!isExchangeNow) {
        ...
        if (isExchangeNow) {
          return yield this.doExchange(org_Id, clientConfig).then((exchRes) => {
            if (Number(exchRes == null ? void 0 : exchRes.code) < 0) {
              ...
              try {
                if (fs2.existsSync(`${baseFolderPath}/${org_Id}/temp_${publicFile}`)) {
                  fs2.renameSync(
                    `${baseFolderPath}/${org_Id}/temp_${publicFile}`,
                    `${baseFolderPath}/${org_Id}/${publicFile}`
                  );
                }
                if (fs2.existsSync(`${baseFolderPath}/${org_Id}/temp_${privateFile}`)) {
                  fs2.renameSync(
                    `${baseFolderPath}/${org_Id}/temp_${privateFile}`,
                    `${baseFolderPath}/${org_Id}/${privateFile}`
                  );
                }
                fs2.writeFileSync(`${baseFolderPath}/${org_Id}/${initFile}`, JSON.stringify(clientConfig));
              } catch (error) {
  ...
  static update(license_Key = "", org_Id = "", assignType = "update") {
    return __async(this, null, function* () {
      ...
      if (fs2.existsSync(orgInitFile)) {
        let fileData = fs2.readFileSync(orgInitFile, "utf8");
...
License.timeZone = moment.tz.guess();
License.doExchange = (..._0) => __async(_License, [..._0], function* (org_Id = _License.org_Id, clientData) {
  try {
    ...
    if (clientData) {
      ...
      let _public_Key = fs2.existsSync(tempPublicFilePath) ? yield fs2.readFileSync(tempPublicFilePath, "utf8") : yield fs2.readFileSync(publicFilePath, "utf8");
      ...
      return yield axios.post(`${_doExchangeApi}`, apiBody, {
        headers: {
        ...
        if (((_a = res.data) == null ? void 0 : _a.resultCode) == 1) {
          fs2.writeFileSync(`${baseFolderPath}/${org_Id.toString().trim()}/${serverFile}`, ((_b = res.data) == null ? void 0 : _b.data) || "");
...
License.getLicense = (org_Id = "", clientData) => __async(_License, null, function* () {
  try {
    ...
    let licenseUrl = "";
    return yield axios.post(`${licenseServerAPI}`, apiBody, {
      headers: {
        "Content-Type": "application/json"
      }
    }).then((res) => {
...
License.checkValidKey = (license_Key = "", baseUrl = "") => __async(_License, null, function* () {
  try {
    ...
    return yield axios.get(`${licenseServerAPI}`, {
      headers: {
        "Content-Type": "application/json"
      }
    }).then((res) => {
...
License.checkPreinit = (org_Id = "") => __async(_License, null, function* () {
  let isInitFile = fs2.existsSync(`${baseFolderPath}/${org_Id}/${initFile}`) || false;
  let isPublicFile = fs2.existsSync(`${baseFolderPath}/${org_Id}/${publicFile}`) || false;
  let isPrivateFile = fs2.existsSync(`${baseFolderPath}/${org_Id}/${privateFile}`) || false;
  return { isInitFile, isPublicFile, isPrivateFile };
});
License.checkExchangeFiles = (org_Id = "") => __async(_License, null, function* () {
  let isServerFile = fs2.existsSync(`${baseFolderPath}/${org_Id}/${serverFile}`) || false;
  let isLicenseFile = fs2.existsSync(`${licenseBaseFolder}/${org_Id}/${licenseFile}`) || false;
...
License.readFileAndParse = (org_Id = "") => __async(_License, null, function* () {
  let fileData = fs2.readFileSync(`${baseFolderPath}/${org_Id}/${initFile}`, "utf8");
  const parseData = JSON.parse(fileData);
  return parseData || null;
});
License.extractLicense = (org_Id = "") => __async(_License, null, function* () {
  ...
  try {
    ...
    if (fs2.existsSync(filePath)) {
      let _encryptedLicense = yield fs2.readFileSync(filePath, "utf8");
...
License.delete = (org_Id = "") => __async(_License, null, function* () {
  ...
  try {
    ...
    if (fs2.existsSync(orgInitFile)) {
      let fileData = fs2.readFileSync(orgInitFile, "utf8");
      const parseData = JSON.parse(fileData);
      return yield axios.delete(`${parseData == null ? void 0 : parseData.baseUrl}/sdk/api/delete/${org_Id}`, {
        headers: {
          "Content-Type": "application/json"
        }
      }).then((res) => {
        ...
        if (((_a = res.data) == null ? void 0 : _a.resultCode) == 1) {
          try {
            ...
            if (fs2.existsSync(`${licenseBaseFolder}/${org_Id}/${licenseFile}`)) {
              fs2.unlink(`${licenseBaseFolder}/${org_Id}/${licenseFile}`, () => {
              });
            }
          } catch (error) {
...
(function() {
  return __async(this, null, function* () {
    function readDirectories(directoryPath) {
      const subFolders = [];
      const contents = fs2.readdirSync(directoryPath);
      for (const item of contents) {
        const itemPath = path.join(directoryPath, item);
        if (fs2.statSync(itemPath).isDirectory()) {
    ...
    cron.schedule(midnight, () => __async(this, null, function* () {
      try {
        ...
        for (const orgId of subFolders) {
          ...
          if (fs2.existsSync(orgInitFile)) {
            ...
            const parseData = JSON.parse(fileData);
            parseData.device = yield getDeviceDetails();
            if (parseData && (parseData == null ? void 0 : parseData.licenseKey) && (parseData == null ? void 0 : parseData.licenseKey) !== "" && fs2.existsSync(`${baseFolderPath}/${orgId}/${serverFile}`)) {
              try {
                ...
                yield axios.post(`${licenseServerAPI}`, apiBody, {
                  headers: {
                    "Content-Type": "application/json"
                  }
